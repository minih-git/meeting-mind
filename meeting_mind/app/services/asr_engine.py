...
class ASREngine:
    ...
    async def enqueue_audio(self, session_id: str, audio_chunk: bytes, is_final: bool = False):
    ...
    def _process_stream(
        ...
        """
        logger.debug(f"[{session_id[:8]}] inference_stream: {len(audio_chunk)} bytes, is_final={is_final}")
        ...
        for seg in vad_segments:
            ...
            if end_ms != -1:
                ...
                if start_ms_stored != -1:
                    ...
                    if start_byte < 0:
                        # 说明开始点已经被移出 buffer 了
                        logger.warning(f"[{session_id[:8]}] 片段开始点已丢失 (延迟: {-start_byte} bytes)")
                        start_byte = 0 
                    ...
                    if end_byte > buffer_len and end_byte - buffer_len < 3200:
                         logger.debug(f"[{session_id[:8]}] VAD 结束点微调: {end_byte} -> {buffer_len}")
                         end_byte = buffer_len
                    ...
                    if end_byte <= buffer_len:
                        ...
                        if len(segment_audio) > 16000 * 0.2 * 2: # 忽略极短片段 (<200ms)
                            ...
                            try:
                                ...
                                seg_audio_np = nr.reduce_noise(y=seg_audio_np, sr=16000, prop_decrease=0.8)
                            except Exception as e:
        ...
        if current_start_ms == -1 and buffer_len_bytes > 16000 * 2 * 2: # 2 seconds
             ...
             logger.info(f"[{session_id[:8]}] Buffer 积压 ({buffer_len_bytes} bytes)，强制触发语音开始: {buffer_start_ms:.0f}ms")
        ...
        if current_start_ms != -1:
            ...
            current_abs_end_byte = session_cache["buffer_offset_bytes"] + len(session_cache["audio_buffer"])
            ...
            if now - last_partial_time > 0.5 and duration_ms > 500: # Every 500ms, if segment > 500ms
                ...
                if len(partial_audio) > 16000 * 0.2 * 2:
                    ...
                    try:
                        ...
                        if isinstance(asr_res, list) and len(asr_res) > 0:
                            ...
                            import re
                            partial_text = re.sub(r"<\|.*?\|>", "", partial_text).strip()
            ...
            if duration_ms > MAX_DURATION_MS:
                logger.info(f"[{session_id[:8]}] 检测到长语音 ({duration_ms:.0f}ms)，强制断句")
                ...
                if end_byte > start_byte:
                    ...
                    if len(segment_audio) > 16000 * 0.2 * 2:
                        ...
                        if asr_text:
                            logger.info(f"[{session_id[:8]}] 识别结果 (Force): {speaker_id} - {asr_text}")
                            results.append({
                                "text": asr_text,
                                "speaker_id": speaker_id,
