    ...
    async with websockets.connect(WS_URL) as ws:
        ...
        with wave.open(AUDIO_FILE, "rb") as wf:
            ...
            if wf.getnchannels() != 1 or wf.getsampwidth() != 2 or wf.getframerate() != 16000:
                print(f"Warning: Audio format mismatch. Expected 1ch 16bit 16000Hz. Got {wf.getnchannels()}ch {wf.getsampwidth()*8}bit {wf.getframerate()}Hz")
            
            data = wf.readframes(CHUNK_SIZE // 2) # readframes takes number of frames, not bytes
            total_bytes = 0
            ...
            while len(data) > 0:
                ...
                print(f"Sent {len(data)} bytes (Total: {total_bytes})")
