...
const HistoryPage = () => {
  ...
  const analyzeMeeting = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/v1/meetings/${selectedMeeting.id}/analyze`, {
        method: 'POST'
      });
  ...
  const retranscribeMeeting = async () => {
    if (!window.confirm('重新转写将覆盖当前的对话记录，确定要继续吗？这可能需要几分钟时间。')) {
      return;
    }

    try {
      setLoading(true);
      const res = await fetch(`/api/v1/meetings/${selectedMeeting.id}/retranscribe`, {
        method: 'POST'
      });
    ...
    return (
      <div className="history-detail">
        <div className="history-header">
          <button className="back-btn" onClick={() => navigate('/history')} title="返回列表">
            ←
          </button>
          <div style={{ flex: 1, marginLeft: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        {selectedMeeting.ai_analysis && (
          <div className="ai-analysis-section" style={{
            ...
            <h3 style={{ marginTop: 0, color: '#4facfe' }}>✨ AI 会议分析</h3>

            <div className="analysis-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
              <div className="analysis-card">
                <h4>📝 会议总结</h4>
                <p style={{ fontSize: '0.9rem', lineHeight: '1.5' }}>{selectedMeeting.ai_analysis.summary}</p>
              </div>
              <div className="analysis-card">
                <h4>💡 关键要点</h4>
        ...
        <div className="transcript-area">
          <h3 style={{ marginTop: '10px', color: '#4facfe', marginBottom: '15px' }}>💬 对话记录</h3>
  ...
  return (
    <div className="history-container">
      <div className="history-header">
        <button className="back-btn" onClick={() => navigate('/home')} title="返回录音">
          ←
        </button>
        <h2>历史记录</h2>
